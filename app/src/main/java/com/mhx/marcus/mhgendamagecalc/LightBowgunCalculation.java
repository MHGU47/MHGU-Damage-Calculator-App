...
public class LightBowgunCalculation extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    ...
    float[] BulletGeyser = {20f, 10f, 39f, 30f, 60f, 60f};
    float[] ChargedShot = {0.2f, 0.4f, 0.6f};
    float[] ChosenHunterArt;
    String[] HunterArtsLevels_BG = {"Level I - Fixed Damage", "Level I - Fire Damage",
            "Level II - Fixed Damage", "Level II - Fire Damage",
            "Level III - Fixed Damage", "Level III - Fire Damage"};
    String[] HunterArtsLevels_CS = {"Level I - Per shot", "Level II - Per shot", "Level III - Per shot"};

    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_light_bowgun_calculation);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        if(ArtText.equals("Bullet Geyser")){
            getHunterArt("Bullet Geyser");
        }
        else if(ArtText.equals("Charged Shot")){
            getHunterArt("Charged Shot");
        }
    ...
    public void Calculate(){
        ...
        Calculate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                TextViewIDsNames = new String[]{"LBGAttack_1_Name", "LBGAttack_2_Name", "LBGAttack_3_Name"
                        , "LBGAttack_4_Name", "LBGAttack_5_Name", "LBGAttack_6_Name"};
                AllTextViewIDs = new String[]{"LBGAttack_1_Name", "LBGAttack_2_Name", "LBGAttack_3_Name",
                        "LBGAttack_4_Name", "LBGAttack_5_Name", "LBGAttack_6_Name", "LBGAttack_1",
                        "LBGAttack_2", "LBGAttack_3", "LBGAttack_4", "LBGAttack_5", "LBGAttack_6"};
                ...
                if (!ChosenArt.equals("-None-")){
                    MotionCheck = 1;
                    if(ChosenArt.equals("Bullet Geyser")){
                        ChosenHunterArt = BulletGeyser;
                    }
                    else{
                        ChosenHunterArt = ChargedShot;
                    }
                }
                //-End-
                ...
                TextView Damage = (TextView) findViewById(R.id.DamageInputLBG);
                TextView Affinity = (TextView) findViewById(R.id.AffinityInputLBG);

                List<String> HitzoneCatchList = Arrays.asList("Head", "Chin", "Horn", "NONE");
                List<String> ShotCatchList = Arrays.asList("Pierce", "Normal", "Elemental");
                ...
                Spinner ArtSelectText = (Spinner) findViewById(R.id.HunterArtSelect);
                String ShotText = ShotSelectText.getSelectedItem().toString();
                String StyleText = StyleSelectText.getSelectedItem().toString();
                String ArtText = ArtSelectText.getSelectedItem().toString();

                if(!ShotCatchList.contains(ShotType) && RapidFireCheck.isChecked()){
                    Snackbar.make(view, "This shot type isn't usable in Rapid Fire", Snackbar.LENGTH_LONG)
                            .setAction("Action", null).show();
                    return;
                }
                ...
                if(!ArtText.equals("-None-") && RapidFireCheck.isChecked()){
                    Snackbar.make(view, "Rapid Fire doesn't work with Hunter Arts", Snackbar.LENGTH_LONG)
                            .setAction("Action", null).show();
                    return;
                }

                if(!ArtText.equals("-None-") && AerialShotSelect.isChecked()){
                    Snackbar.make(view, "Aerial Shot doesn't work with Hunter Arts", Snackbar.LENGTH_LONG)
                            .setAction("Action", null).show();
                    return;
                }
                ...
                Skills.setRapidFireModifier(RapidFireCheck.isChecked(), ShotType);
                ...
                /*'M' is the new variable in this block of code and is used to call the
                 ...
                 class.*/
                M = new MonsterCalculation(LightBowgunCalculation.this,
                        ChosenMonster + "",
                        ChosenMonster + "_StaggerLimits",
                        HitzoneGroup + "Hitzones",
                        ChosenHitzone);

                M.getHitzones(LightBowgunCalculation.this, Skills, WeaknessExploitCheck.isChecked());
                float TrueRaw = Skills.getTrueRaw(RawDamage * 1.3f, RawAffinity, SkillCheck);

                if(MotionCheck == 1){
                    if(ChosenArt.equals("Bullet Geyser")){
                        M.setElmHitzoneValue(LightBowgunCalculation.this,
                                ChosenMonster + "ElmHitzones_Fire");

                        for (int i = 0; i < BulletGeyser.length; i++) {
                            TrueRaw = BulletGeyser[i];

                            //Hitzone Modification - Start

                            float HitzoneElm = ChosenHunterArt[i] * M.getElmHitzoneValue() / 100;
                            ...
                            textviews[i].setText(HunterArtsLevels_BG[i]);
                            textviews[i].setVisibility(View.VISIBLE);

                            textviews[i] = (TextView) findViewById(getResources().getIdentifier(TextViewIDsAttacks[i], "id", getPackageName()));
                            if(HunterArtsLevels_BG[i].contains("Fire")){
                                textviews[i].setText(String.format("%s", Math.round(HitzoneElm)));
                            }
                            else{
                                textviews[i].setText(String.format("%s", Math.round(TrueRaw)));
                            }
                            textviews[i].setVisibility(View.VISIBLE);
                        }
                    }
                    else{
                        for (int i = 0; i < ChargedShot.length; i++) {
                            TrueAttack = TrueRaw * ChosenHunterArt[i] * M.getRawHitzoneValue() / 100;

                            textviews[i] = (TextView) findViewById(getResources().getIdentifier(TextViewIDsNames[i], "id", getPackageName()));
                            textviews[i].setText(HunterArtsLevels_CS[i]);
                            textviews[i].setVisibility(View.VISIBLE);
                    ...
                    if (AerialShotSelect.isChecked() && SelectedStyle.equals("Aerial"))
                        ...
                        Toast.makeText(LightBowgunCalculation.this, "Please select Aerial Style/'Airborne' to use 'Aerial Shot'",
                    ...
                    if(AirborneCheck.isChecked() && AerialShotSelect.isChecked() && SelectedStyle.equals("Aerial") && SkillCheck)
                        ...
                        if(!AerialShotSelect.isChecked())
                            Toast.makeText(LightBowgunCalculation.this, "Please select 'Aerial Shot' to use 'Airborne'",
                                    Toast.LENGTH_SHORT).show();
                        Toast.makeText(LightBowgunCalculation.this, "Please select Aerial Style to use 'Airborne'",
                                Toast.LENGTH_SHORT).show();
                        return;
                    }
                    ...
                    if (ShotType.equals("Triblast S") || ShotType.equals("Crag") || ShotType.equals("Clust")) {
                        ...
                        for (int i = 0; i < MotionAtk.length; i++) {
                            ...
                            //Hitzone Modification - Start
                            if(i == 1){
                                M.alterHitzones(LightBowgunCalculation.this, ChosenMonster);
                            }
                            float HitzoneRaw = (TrueRaw * M.getRawHitzoneValue()) / 100;
                        ...
                        for (int i = 0; i < MotionAtk.length; i++) {
                            ...
                            if (MotionAtk[i] != 0f) {
                                if (ShotType.equals("Normal") || ShotType.equals("Pierce")) {
                                    ...
                                    M.getHitzones_Stinger(LightBowgunCalculation.this, Skills);
